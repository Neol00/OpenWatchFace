# Mobvoi TicWatch S2 / E2 (tunny) install guide
...
| Display | **400×400** round AMOLED (1.39"), MSM DSI **command mode** (MDP3 DMA_P pipe takeover) |
| Touch | FocalTech FTS (I²C, BLSP1 QUP5 @ `0x38`), interrupt-latched on TLMM GPIO 13 |
| Buttons | **One** the single side pusher (PM8916 KPDPWR). No crown, no rotating bezel |
| RTC | PM8916 PMIC RTC (battery-backed, read over SPMI) |
| Radios | **WCN3620** (WCNSS/Pronto): WiFi WPA2 + DHCP + NTP + HTTP, BLE via NimBLE (iPhone pairing + ANCS notifications). HTTPS via mbedTLS (SoC hardware RNG), proven: over-the-air updates download, verify and install. The radio stays resident and idle through deep sleep. The NFC chip (ST21NFC) is untouched |
| IMU | ST **LSM6DS3** on bit-banged SPI (GPIO 8–11), hardware pedometer for the step counter |
| Heart rate | PixArt **PAH8011** on I²C (GPIO 6/7). Detected and configured, but **it never streams samples and the green LEDs never light**; the Heart app opens and reads nothing |
...
> ### The S2 is the C2 with a bigger screen
>
> The S2's own device tree, pulled from its boot partition, differs from the
> [TicWatch C2's](ticwatch-c2.md) by exactly the parts list: a 400×400 panel
> where the C2 has 360×360, an NFC node, and one pin. Every SoC address, the
> radio, the sensors, the button, the partition table and even the WiFi
> calibration blob are the same. The port is therefore the C2 board header
> plus three overrides (name, panel size, touch scale), and the build sets
> **both** board defines so every C2 driver applies unchanged.
>
> That also means the two images are **not interchangeable**: a C2 image on
> the S2 draws a 360×360 framebuffer into a 400×400 pipe and scales touch
> wrong. Always build and flash the S2 image on the S2.
...
| DTB | `snapdragon-port/dtbs/tunny-stock.dtb` **in this repo, and the packer picks it up by default** |
| NV blob | optional, for WiFi: `snapdragon-port/firmware/s2/wcnss_nv.c` generated from *your* watch (below) |
...
ls build/s2/         # result: build/s2/owf-boot.img
```
...
> non-destructive refusal. `mk-bootimg-s2.sh` defaults to the in-repo tunny DTB.
...
> the S2's own stock boot partition with `tools/extract-dtb.py` (board-id
> `0x08 0x105`). The decompiled `tunny-stock.dts` sits next to it. If you want
> your own watch's copy, extract it from your `boot` partition dump the same
> way (the `/sys/firmware/fdt` node is not readable without root on Wear OS).
...
sh snapdragon-port/tools/mk-wcnss-nv.sh WCNSS_qcom_wlan_nv.bin wifimac.ini s2
```

That writes `snapdragon-port/firmware/s2/wcnss_nv.c`, which the build script
...
`no firmware/s2/wcnss_nv.c - NV download will be skipped` and WiFi is off;
BLE shares the same radio firmware and the same blob. (The S2's blob turned out
byte-identical to the C2's; only the MAC differs, so a C2 owner's blob with
your own MAC works too.)
...
works fine. Firefish TWRP boots on this watch and is what was used on the S2 
...
Partitions are addressed as `/dev/block/mmcblk0pN`. The S2 has the same
37-partition map as the C2:
...
Confirm with `adb shell "cat /proc/partitions"` and, if your TWRP has them,
`adb shell "ls -l /dev/block/platform/*/by-name/"`. Then dump:

```sh
mkdir -p s2-stock-backup && cd s2-stock-backup
adb shell "dd if=/dev/block/mmcblk0" > s2-emmc-full.img        # whole eMMC, ~3.6 GB
adb shell "dd if=/dev/block/mmcblk0p33" > s2-boot.img
adb shell "dd if=/dev/block/mmcblk0p24" > s2-persist.img
adb pull /persist/WCNSS_qcom_wlan_nv.bin
adb pull /persist/wifimac.ini
```

Check the sizes (`ls -l`, nothing may be 0 bytes) and compare md5s against
`adb shell "md5sum /dev/block/mmcblk0p33"` before trusting them. Expect
`userdata` and `cache` to differ from the device because TWRP itself mounts
and touches them; every other partition should match exactly. Do not mount
`system` read-write from TWRP while you are verifying, that changes its md5.
...
```sh
fastboot flash boot owf-ticwatch-s2.img
fastboot reboot
```

or from the TWRP root shell, which is how the S2 in this project was installed:

```sh
adb push owf-ticwatch-s2.img /tmp/owf.img
...
padding. **Restore Wear OS** the same way with your `s2-boot.img`.
...
At 400×400 the S2 sits between the C2 (360) and the Gen 6 (416). The firmware
derives its UI tier from the panel height, so the watchface dial, launcher
grid, app columns and player transport all pick their own sizes from it with
no S2-specific numbers anywhere. The boot log's framebuffer line is the check:
it must say 400×400, read back from what the bootloader programmed into DMA_P.
...
- **Steps** come from the LSM6DS3's hardware pedometer. **Heart rate** does
  not: the PAH8011 answers on the bus and accepts the full configuration
  recovered from the modem DSP driver, but has never produced a sample or lit
  its LEDs. This unit is the reference for that work because its sensor is
  known good under Wear OS. Parked.
...
- **Updates arrive over WiFi, confirmed on the Gen 4; same code here.** WiFi &
  BLE → Check for updates asks GitHub for the latest release (tag compared
  numerically against the build's version) and, when it carries
  `owf-ticwatch-s2-tunny-<version>.img`, Install update downloads it (resuming
  with HTTP ranges if the link stalls), verifies the SHA-256 from the release's
  `SHA256SUMS`, checks the boot header and writes it into `boot`, header block
  last. After the first cable flash the USB link is only needed for the log.
  The release flow is in the [README](../../README.md#software-updates-over-the-air).
...
| `fastboot boot` fails with *dtb not found* | The image was packed without a DTB. Re-run `tools/mk-bootimg-s2.sh`, which appends the in-repo tunny DTB by default. Nothing ran, nothing was damaged. |
| Watch resets a few seconds into every boot | You built without `-DWDOG_TRACE`. |
| Face is drawn small in the top-left, touch lands in the wrong place | You are running a **C2** image. Build with `build-owf-image-s2.sh` and pack with `mk-bootimg-s2.sh`. |
| Screen stays black but the watch is clearly alive | aboot handed over a dark panel. The takeover path inherits the bootloader's display state; there is no blind DSI fallback here. |
| WiFi log ends at `HAL_START` with no response | No NV blob embedded. Generate `firmware/s2/wcnss_nv.c` from your watch's `persist` and rebuild. |
...
- **The port is three overrides on the C2.**
  [`snapdragon-port/baremetal/boards/ticwatch_s2.h`](../../snapdragon-port/baremetal/boards/ticwatch_s2.h)
  includes the C2 header and redefines the platform name, the panel size and
  the touch coordinate range; the app-side
  [`OpenWatchFace/board_ticwatch_s2.h`](../../OpenWatchFace/board_ticwatch_s2.h)
  does the same for the name, the OTA key and the LCD size. Everything else is
  [the C2 page](ticwatch-c2.md).
- **The E2 is the same board** (tunny) with a different case. It is untested
  here but there is no known reason for it to differ.
