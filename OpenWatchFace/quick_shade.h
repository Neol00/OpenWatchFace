...
/* Shade height (also the full pull distance) and the top grab-strip height are
 * PER-BOARD. The S3-2.06 keeps its tuned pixel values. The C6-1.47 is a tall
 * narrow panel where UI_PX(368)=~154px would only pull down to ~half the 320px
...
static void quick_shade_init(void) {
  ...
  lv_obj_set_style_border_width(qs_panel, UI_PX(2), 0);    // thin divider at the bottom edge only
...
#if QS_BTNS_VERTICAL
  // C6 (narrow): the bigger header + value collide on one row, so put the value
  // BELOW the header instead of top-right.
  lv_obj_align_to(qs_val, qs_hdr, LV_ALIGN_OUT_BOTTOM_LEFT, 0, 8);
#else
  lv_obj_align(qs_val, LV_ALIGN_TOP_RIGHT, UI_PX(-66), UI_PX(54));
#endif

  // the slider (10..255 so the panel itself never goes pitch black)
  qs_slider = lv_slider_create(qs_panel);
  lv_obj_set_height(qs_slider, UI_PX(16));
  lv_slider_set_range(qs_slider, QS_BRIGHT_MIN_PCT, 100);   // percent of full (100% = raw 255)
#if QS_BTNS_VERTICAL
  // C6 (tall panel): slider sits below the header near the top; toggles fill below.
  lv_obj_set_width(qs_slider, LV_PCT(80));
  lv_obj_align(qs_slider, LV_ALIGN_TOP_MID, 0, 92);
#else
  lv_obj_set_width(qs_slider, UI_PX(330));
  lv_obj_align(qs_slider, LV_ALIGN_CENTER, 0, UI_PX(-38));   // sits higher to leave room for the toggles
#endif
  lv_obj_set_style_bg_color(qs_slider, lv_color_hex(ui_accent_hex()), LV_PART_INDICATOR);
  lv_obj_set_style_bg_color(qs_slider, lv_color_hex(ui_accent_hex()), LV_PART_KNOB);
...
#if QS_BTNS_VERTICAL
  // C6 (narrow): stack the two toggles vertically in the taller panel — full-width
  // buttons, one above the other. Size them to FIT the panel space below the slider
  // (panel bottom minus the toggles' top anchor minus a margin for the grab handle),
  // so the second button never clips off the bottom.
  const int qs_tog_top  = 132;                 // toggles area starts here (below slider)
  const int qs_tog_gap  = 12;                  // gap between the two buttons
  const int qs_bottom_m = 24;                  // leave room for the grab handle at the panel bottom
  const int qs_avail_h  = QS_PANEL_H - qs_tog_top - qs_bottom_m;
  const int qs_btn_w = (int)(screenWidth * 80 / 100);
  int qs_btn_h = (qs_avail_h - qs_tog_gap) / 2;
  if (qs_btn_h > 64) qs_btn_h = 64;            // don't get absurdly tall on a big panel
  if (qs_btn_h < 36) qs_btn_h = 36;            // floor so they stay tappable
  lv_obj_set_size(toggles, qs_btn_w, qs_btn_h * 2 + qs_tog_gap);
  lv_obj_align(toggles, LV_ALIGN_TOP_MID, 0, qs_tog_top);
  ...
  qs_caf_btn = lv_btn_create(toggles);              // LEFT / TOP
  lv_obj_set_size(qs_caf_btn, qs_btn_w, qs_btn_h);
  lv_obj_set_style_radius(qs_caf_btn, UI_PX(16), 0);
  ...
  qs_mute_btn = lv_btn_create(toggles);             // RIGHT / BOTTOM
  lv_obj_set_size(qs_mute_btn, qs_btn_w, qs_btn_h);
  lv_obj_set_style_radius(qs_mute_btn, UI_PX(16), 0);
  ...
  lv_obj_set_width(qs_mute_icon, UI_PX(40));
  ...
  lv_obj_set_size(handle, UI_PX(54), UI_PX(5));
  lv_obj_align(handle, LV_ALIGN_BOTTOM_MID, 0, UI_PX(-10));
  lv_obj_set_style_bg_color(handle, lv_color_hex(0x555555), 0);
  lv_obj_set_style_bg_opa(handle, LV_OPA_COVER, 0);
  lv_obj_set_style_radius(handle, UI_PX(3), 0);
