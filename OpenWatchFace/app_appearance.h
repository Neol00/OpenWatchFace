...
static void appr_refresh_selection(void) {
  ...
  for (uint8_t i = 0; i < APPR_COLOR_COUNT; i++) {
    ...
    lv_obj_set_style_border_width(appr_swatches[i], sel ? UI_PX(4) : UI_PX(2), 0);
  }
}
...
static void app_open_appearance(void) {
...
#else
  // S3-2.06: the ORIGINAL fixed geometry. The toggle rows inside use fixed pixel
  ...
  lv_obj_set_style_pad_top(appr_hdr, UI_PX(4), 0);
  ...
  lv_obj_set_style_pad_top(appr_wf_hdr, UI_PX(12), 0);
  ...
  lv_obj_set_size(tail, UI_PX(1), UI_PX(24));
}
