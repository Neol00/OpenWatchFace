...
> [`dtbs/sda429-hoki-decompiled.dts`](dtbs/sda429-hoki-decompiled.dts) (11,779 lines,
...
| WiFi (WCNSS) | **blocked** | SMEM, SCM, SMD/RPM votes and the PAS loader all work, but TrustZone rejects `pas_init_image(6)` with -13 on both Gen 6 units before it hashes the firmware (paused 2026-09-03). The same stack runs WiFi + BLE on the Wear 2100 watches. |
| BLE (WCN3990 over UART, *not* WCNSS on this SoC) | **not implemented** | separate from the WiFi path: `qca,wcn3990` power GPIOs + HCI UART + firmware from the `bluetooth` partition |
