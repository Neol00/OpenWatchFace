...
The real OpenWatchFace firmware runs on the Gen 4 as a daily watch: UI on
the glass, touch and crown, storage, WiFi and BLE, over-the-air updates, dual
core, and a deep sleep that power-collapses the whole CPU cluster (about 6 mA
on the sibling C2, stock territory). The per-subsystem table is under
[Subsystem status](#subsystem-status); the user-facing install guide is
[docs/devices/fossil-gen4.md](../docs/devices/fossil-gen4.md).
...
```sh
fastboot boot snapdragon-port/baremetal/build/gen4-owf/owf-gen4-<TAG>.img
```
