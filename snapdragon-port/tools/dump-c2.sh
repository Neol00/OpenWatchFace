#!/bin/sh
# dump-c2.sh — full backup of a Mobvoi TicWatch C2 / C2+ (skipjack, WG12036)
# before its stock Wear OS install is replaced, plus the bring-up artifacts.
#
# Byte-for-byte the same job as dump-gen4.sh, and deliberately the same script:
# the C2 is the Gen 4's SoC (APQ8009W) from a different vendor, so the eMMC
# controller, the by-name layout and the probing all match. Only the way you
# get a root shell differs.
#
# RUN THIS ON THE HOST, with the watch showing an adb shell that has ROOT.
# On this watch the practical way is to RAM-boot TWRP (skipjack builds exist
# on XDA), which touches no partition:
#
#     fastboot boot twrp-skipjack.img
#     adb wait-for-device && adb shell id      # expect uid=0(root)
#     sh snapdragon-port/tools/dump-c2.sh
#
# NOTE the word `boot`, not `flash`. RAM-booting runs the image and leaves
# every partition exactly as it was; a power cycle undoes it completely.
#
# The device tree is NOT readable without root on Wear OS (/sys/firmware/fdt
# is denied), so there is no shortcut: the DTB comes out of the boot partition
# dump this script takes (tools/extract-dtb.py boot.img). Take the backup
# before anything is flashed, because after that the stock images are gone.
#
# This script is READ-ONLY on the device: it only ever reads block devices.
# Nothing is flashed, erased or written.
#
#   sh snapdragon-port/tools/dump-gen4.sh [outdir]
#
# Default outdir: snapdragon-port/dumps/gen4-<date>.
#
...
OUT="${OUT:-$(dirname "$0")/../dumps/c2-$(date +%Y%m%d)}"
...
case "$ID" in
  ...
  *) echo
     ...
     echo "RAM-boot TWRP first:  fastboot boot twrp-skipjack.img"
     exit 1 ;;
esac

echo "[dump] 1/5 merged device tree"
adb pull /proc/device-tree "$OUT/device-tree" >/dev/null 2>&1 \
  || echo "     (blocked)"
# The same tree as one binary blob. /proc/device-tree is a directory of files,
# fine for grepping but awkward to feed to dtc; /sys/firmware/fdt is the flat
# blob the kernel actually booted with, which `dtc -I dtb -O dts` reads
# directly. Both are kept because each is easier for a different question.
echo "[dump]     raw flattened tree (/sys/firmware/fdt)"
adb pull /sys/firmware/fdt "$OUT/fdt.dtb" >/dev/null 2>&1 \
  && echo "     -> $OUT/fdt.dtb  (dtc -I dtb -O dts fdt.dtb > fdt.dts)" \
  || echo "     (unavailable)"

echo "[dump] 2/5 kernel + platform info"
...
echo "[dump] 3/5 locating the partition table"
...
echo "[dump] 4/5 pulling partitions"
: > "$OUT/manifest.tsv"
...
# Restoring this TicWatch C2 to stock

These images were taken with `tools/dump-c2.sh`. Check `manifest.tsv`
...
- If a partition is not writable by fastboot, RAM-boot TWRP and use
