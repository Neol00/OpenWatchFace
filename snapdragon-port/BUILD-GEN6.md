...
| DTB | `snapdragon-port/dtbs/sda429-hoki.dtb` (checked in) |
...
```sh
fastboot flash boot snapdragon-port/baremetal/build/gen6-owf/owf-real-<TAG>-recovery.img
```
...
| `-DSMEM_DIAG` | shared-memory (SMEM) header, partition table and item inventory at boot (the first thing to read when the radio does not start) |
| `-DWIFI_DIAG` | TZ `is_call_available` probe (`scm.c`), an SMD open + no-op request to the RPM (`smd.c`), then the full WCNSS boot attempt (`wcnss.c`: firmware from the `modem` partition, RPM rail votes, Iris XO, `pas_auth_and_reset`), run once after USB enumerates |
| `-DSMEM_SCAN` | the late (post-USB-enumeration) SMEM probe: prints each address before reading it, so a read that resets the watch still names itself |
