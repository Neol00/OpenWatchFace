...
/* The shared LVGL glue drives the C2/Gen 4 double-buffered, CPU1-served frame
 * push (fb_mdp3.c). This board is single-buffered and pushes on core 0, so the
 * second buffer is absent and a "flush this buffer" is just the normal flush. */
void *fb_ptr2(void) { return 0; }
void fb_flush_buf(const void *buf) { (void)buf; fb_flush_all(); }
