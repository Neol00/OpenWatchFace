/* storage_gen6.c — persistent storage inside the userdata partition.
 *
 * Born on the Gen 6 (hence the name); since 2026-08-30 it is the storage core
 * for EVERY eMMC board -- the Fossil Gen 4 and the TicWatch C2 build it too.
 * Nothing in here is board-specific: the userdata extent comes from a GPT
 * lookup at boot, the eMMC access is fenced by sdhci_msm.c, and the board
 * differences (clocks, pads, card LDO) live behind the PLAT_* defines the
 * board header declares next to PLAT_HAVE_EMMC_STORAGE.
 *
 ...
 */
#include "platform.h"
#if defined(PLAT_HAVE_EMMC_STORAGE)
...
#define REG_FFAT_NBLK 2097152u       /* 1 GiB  (the default/maximum) */
/* Smallest FFAT region we will create: 256 MiB. The Gen 6 has 4.65 GiB of
 * userdata; the 4 GB-eMMC Gen 4 / C2 carry well over 1 GiB there too, so the
 * full 1 GiB is the normal case -- the region only SHRINKS when a watch's
 * userdata really is smaller, instead of refusing storage outright. */
#define REG_FFAT_MIN_NBLK 524288u
...
static uint32_t s_ffat_nblk = REG_FFAT_NBLK;   /* actual FFAT region size */
static int      s_ok;
static uint32_t s_boot_count;
static int      s_foreign;           /* userdata still holds a Wear OS volume: read-only */

int storage_ok(void) { return s_ok; }

int storage_foreign(void) { return s_foreign; }
uint32_t storage_boot_count(void) { return s_boot_count; }
...
{
    const uint32_t n[3] = { REG_BB_NBLK, REG_NVS_NBLK, s_ffat_nblk };
    return (s_ok && id < 3u) ? n[id] : 0u;
}

int storage_init(void)
{
    if (s_foreign) return -1;           /* decided once per boot; do not re-probe on every NVS call */
    ...
    if (s_nblk < REG_FFAT_LBA + REG_FFAT_MIN_NBLK) {
        con_puts("storage: userdata too small\n");
        STG_DIAG(0xFF0000u);
        return -1;
    }
    s_ffat_nblk = s_nblk - REG_FFAT_LBA;
    if (s_ffat_nblk > REG_FFAT_NBLK) s_ffat_nblk = REG_FFAT_NBLK;
    ...
    if (!(sb->magic0 == SB_MAGIC0 && sb->magic1 == SB_MAGIC1)) {
        /* FOREIGN-FILESYSTEM GUARD (2026-09-06). Not ours: before claiming
         * the partition, make sure it is not still Wear OS's userdata. A
         * `fastboot boot` of this firmware on a watch that still has the
         * stock OS installed used to write our superblock, blackbox and NVS
         * regions straight over Android's filesystem, which corrupted the
         * stock install and needed a recovery wipe to get Wear OS back.
         * Three signatures cover the cases seen on these watches:
         *   ext4  superblock at byte 1024, magic 0xEF53 at +0x38
         *   f2fs  superblock at byte 1024, magic 0xF2F52010 at +0
         *   FDE   crypto footer in the last 16 KiB, magic 0xD0B5B1C4
         *         (Wear OS encrypts userdata, so the filesystem itself
         *         shows no magic; the footer is what remains readable).
         * If any is present the storage layer goes READ-ONLY for this boot:
         * no superblock, no blackbox, no NVS, no FFat. Settings then live
         * in RAM only, which is the right behaviour for a RAM boot. To
         * install for real, erase userdata first (fastboot erase userdata,
         * or a recovery wipe); the next boot then creates our volume. */
        static uint8_t probe[512];
        int foreign = 0;
        if (emmc_read_block(s_base_lba + 2u, probe) == 0) {
            uint32_t f2 = (uint32_t)probe[0] | ((uint32_t)probe[1] << 8) |
                          ((uint32_t)probe[2] << 16) | ((uint32_t)probe[3] << 24);
            uint32_t e4 = (uint32_t)probe[0x38] | ((uint32_t)probe[0x39] << 8);
            if (e4 == 0xEF53u) { foreign = 1; con_puts("storage: userdata holds an ext4 filesystem"); }
            else if (f2 == 0xF2F52010u) { foreign = 1; con_puts("storage: userdata holds an f2fs filesystem"); }
        }
        if (!foreign && s_nblk > 32u && emmc_read_block(s_base_lba + s_nblk - 32u, probe) == 0) {
            uint32_t m = (uint32_t)probe[0] | ((uint32_t)probe[1] << 8) |
                         ((uint32_t)probe[2] << 16) | ((uint32_t)probe[3] << 24);
            if (m == 0xD0B5B1C4u) { foreign = 1; con_puts("storage: userdata holds an encrypted Android volume"); }
        }
        if (foreign) {
            con_puts(" (Wear OS) - NOT touching it. Storage is read-only this boot;\n"
                     "storage: settings will not persist. Erase userdata before a real install.\n");
            s_foreign = 1;
            STG_DIAG(0x00FFFFu);            /* cyan: intact stock volume, left alone */
            return -1;
        }
    }
    ...
    if (sb->magic0 == SB_MAGIC0 && sb->magic1 == SB_MAGIC1) {
        ...
        /* An existing volume keeps the size it was formatted with (never
         * grow past what the partition can hold, never shrink a live FAT). */
        if (sb->ffat_lba == REG_FFAT_LBA && sb->ffat_nblk >= REG_FFAT_MIN_NBLK &&
            sb->ffat_nblk <= s_nblk - REG_FFAT_LBA)
            s_ffat_nblk = sb->ffat_nblk;
    } else {
        ...
        sb->ffat_lba = REG_FFAT_LBA; sb->ffat_nblk = s_ffat_nblk;
        s_boot_count = 1;
    ...
    con_puts(" blocks, boot #"); con_putdec(s_boot_count);
    con_puts(", ffat "); con_putdec(s_ffat_nblk / 2048u); con_puts(" MiB\n");
    return 0;
...
static void bb_flush(int force)
{
    if (!s_ok) return;
    uint32_t now = timer_ms();
    if (!force && (uint32_t)(now - s_bb_last_ms) < 2000u) return;
    s_bb_last_ms = now;
...
void blackbox_flush(void) { bb_flush(0); }

/* Force a flush NOW (no rate limit). Called right before every step that can
 * end in a silent reset (TZ calls, first touch of a subsystem block), so the
 * line announcing the step is on eMMC before the step runs. Incremental, so
 * it is a handful of block writes -- cheap enough to call per line. */
void blackbox_sync(void) { bb_flush(1); }

/* Print the tail of the PREVIOUS boot's blackbox at boot. This is the crash
 * forensics path when the watch is POWERED OFF between runs (DDR lost, so the
 * ramlog does not carry over): the eMMC snapshot does. Must run before this
 * boot's first flush overwrites it -- i.e. straight after storage_init(). */
void blackbox_print_previous(void)
{
    if (!s_ok) return;
    uint32_t bb = storage_region_lba(0);
    uint8_t blk[512];
    if (emmc_read_block(bb, blk) < 0) return;
    const struct bb_hdr *h = (const struct bb_hdr *)blk;
    uint32_t cap = h->cap, head = h->head, wrapped = h->wrapped, seq = h->seq, bc = h->boot_count;
    if (h->magic != BB_MAGIC || cap < 32u || cap > (16u * 1024u * 1024u) || head >= cap) {
        con_puts("blackbox: no previous snapshot\n");
        return;
    }
    uint32_t bufcap = cap - 16u;                 /* ramlog struct header is 16 B */
    uint32_t want = 3072u;
    if (!wrapped && head < want) want = head;
    if (want == 0u) { con_puts("blackbox: previous snapshot empty\n"); return; }

    con_puts("blackbox: ---- previous boot #"); con_putdec(bc);
    con_puts(" (flush seq "); con_putdec(seq); con_puts("), last "); con_putdec(want);
    con_puts(" bytes ----\n");
    uint32_t cur_blk = 0xFFFFFFFFu;
    for (uint32_t k = 0; k < want; k++) {
        uint32_t idx = (head + bufcap - want + k) % bufcap;   /* ring index */
        uint32_t pos = 16u + idx;                            /* byte in mirror */
        uint32_t b = pos / 512u;
        if (b != cur_blk) {
            if (emmc_read_block(bb + 1u + b, blk) < 0) { con_puts("\nblackbox: read error\n"); return; }
            cur_blk = b;
        }
        char c = (char)blk[pos % 512u];
        if (c == '\r') continue;
        if (c == '\n' || (c >= 0x20 && c < 0x7F)) con_putc(c);
    }
    con_puts("\nblackbox: ---- end of previous boot ----\n");
}
...
static int stairs_fail(void)
{
    ...
    uint8_t en = 0;
#if defined(PLAT_EMMC_VDD_EN_CTL)
    int l19 = (spmi_read8(PLAT_EMMC_VDD_SID, PLAT_EMMC_VDD_EN_CTL, &en) == 0)
              ? ((en & 0x80u) ? 1 : 0) : -1;
#else
    int l19 = -1; (void)en;
#endif
...
#define STAIR(c) do { wdog_pet(); deadman_kick(); fb_trace(c); timer_delay_ms(300); } while (0)
    ...
    if (emmc_gpt_find("userdata", &s_base_lba, &s_nblk) < 0 &&
        emmc_gpt_find_largest(&s_base_lba, &s_nblk) < 0)
        return stairs_fail();
    if (s_nblk < REG_FFAT_LBA + REG_FFAT_MIN_NBLK) return stairs_fail();
