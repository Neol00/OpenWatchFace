...
/* IDLE-LOAD FIX (2026-09-04): every wait below used to spin at full speed for
 * the length of the wire transfer (a 13-byte FocalTech read at 100 kHz is
 * ~1.3 ms, 30 times a second = ~4 % of the core doing nothing). With the
 * scheduler tick armed, WFI parks the core until the next interrupt (<= 1 ms)
 * and the loop re-checks; the transfer completes in hardware meanwhile. */
extern volatile uint32_t g_tick_armed;
static inline void qup_relax(void) { if (g_tick_armed) __asm__ volatile("wfi"); }
...
int i2c_bus_init(uintptr_t base)
{
    ...
    /* Sensor buses (2026-09-03): QUP1/QUP2 were the modem's; with no modem
     * running the AP owns them. Their GCC clocks are brought up here. */
    if (base == 0x078B5000u) { if (gcc_blsp_qup_i2c_up(0x0200Cu, 0x02008u, "QUP1") < 0) return -1; }
    else if (base == 0x078B6000u) { if (gcc_blsp_qup_i2c_up(0x03000u, 0x03010u, "QUP2") < 0) return -1; }
    /* Charger bus (2026-09-06): the C2/S2 SMB231 + STC3117 hang off QUP4. */
    else if (base == 0x078B8000u) { if (gcc_blsp_qup_i2c_up(0x05000u, 0x05020u, "QUP4") < 0) return -1; }
    else if (base != PLAT_I2C_TOUCH_BASE) {
#else
...
#if defined(PLAT_SOC_MSM8909)
    else if (gcc_blsp_qup4_up() < 0) return -1;
#else
