...
/* Second buffer (2026-09-06): LVGL alternates between the two in direct mode
 * and keeps them in sync itself, so core 0 can render frame N+1 into one while
 * CPU1 is still pushing frame N out of the other. */
static uint32_t s_owned_fb2[PLAT_PANEL_W * PLAT_PANEL_H]
    __attribute__((aligned(4096)));
...
void *fb_ptr2(void) { return (void *)s_owned_fb2; }

/* Push `buf` to the panel. With CPU1's flush service up this RETURNS AS SOON
 * AS THE TRANSFER HAS STARTED: it retires the previous frame first (so the
 * MDP never has two in flight and the buffer LVGL is about to draw into is
 * no longer being read), cleans `buf` out of core 0's D-cache, and hands it
 * over. Without CPU1 it is the old synchronous clean+kick+wait. */
void fb_flush_buf(const void *buf)
{
    if (!s_live) return;
    if (smp_flush_available()) {
        int prev = smp_flush_wait();                 /* frame N-1 done before N starts */
        if (prev == -2) { fb_flush_buf(buf); return; }   /* service died: sync path */
        s_kick_err = prev < 0 ? 1u : 0u;
        fb_flush_region(buf, s_stride * s_h);        /* MUST be core 0: its dirty lines */
        smp_flush_post(buf);
    } else {
        s_kick_err = (mdp3_flush(buf) == 0) ? 0u : 1u;
    }
    s_kick_count++;
    s_last_kick_ms = timer_ms();
}

/* Synchronous full push of the primary buffer (debug traces, bring-up). */
void fb_flush_all(void)
{
    if (!s_live) return;
    fb_flush_buf((const void *)s_fb);
