 ...
 */
#include "platform.h"
#if defined(PLAT_BOARD_FOSSIL_GEN6) || defined(PLAT_SOC_MSM8909)
...
int mpm_arm_pmic_wake(uint64_t wake_cntpct)
{
    ...
    /* vMPM words 0/1 = the 64-bit sleep-clock wake time (mpm-of.c
     * msm_mpm_set); ~0 = no timed wake, exactly what the vendor writes when
     * no timer is pending. Our timed wakes come through the PMIC RTC alarm
     * on this same pin. */
    /* v171: the vendor value is ABSOLUTE QTIMER TICKS (msm_mpm_enter_sleep:
     * wakeup = cntpct + sclk*19.2e6/32768), lo word at +0, hi at +4. This is
     * how the kernel gets a timed wake out of a cluster/XO-shutdown sleep
     * (the QTimer frame IRQ is not an MPM pin); 0 = no timed wake (~0). */
    if (!wake_cntpct) wake_cntpct = ~0ULL;
    mmio_write(MPM_VMPM_BASE + 0u, (uint32_t)wake_cntpct);
    mmio_write(MPM_VMPM_BASE + 4u, (uint32_t)(wake_cntpct >> 32));
...
void mpm_report(void)
{
     ...
     * doorbell, no side effects beyond a bit we put back. */
    {
        ...
        diag_puts(")\n");
        con_puts(g_mpm_ram_live ? "mpm: vmpm write probe LIVE (pmic pin 62 armable)\n"
                                : "mpm: vmpm write probe DEAD -> no system collapse\n");
    }
    diag_flush();
}

/* THE RPM'S WAKE INTERRUPT (v171, 2026-09-08). mpm-of.c msm_mpm_irq():
 * "When the system resumes from deep sleep mode, the RPM hardware wakes up
 * the Apps processor by triggering this interrupt. This interrupt has to be
 * enabled and set as wake for the irq to get SPM out of sleep." Requested
 * IRQF_TRIGGER_RISING (DTS interrupts = <0 0xab 1>). Every system-collapse
 * attempt so far (v153..v161) ran with it disabled. Armed only around a
 * cluster collapse; a guard disables it again if it ever storms. */
volatile uint32_t g_mpm_irq_storm;
static int s_ipc_armed;
static void mpm_ipc_irq(void *arg)
{
    (void)arg;
    g_mpm_irq_n++;
    g_mpm_last_sts0 = mmio_read(MPM_REG(MPM_STATUS, 0));
    ...
    for (unsigned s = 0; s < MPM_REG_WIDTH; s++)
        mmio_write(MPM_REG(MPM_STATUS, s), 0u);
    if (g_mpm_irq_n > 256u) { gic_disable_irq(MPM_IPC_INTID); g_mpm_irq_storm = 1u; }
}
void mpm_ipc_irq_arm(void)
{
    if (g_mpm_irq_storm) return;
    if (!s_ipc_armed) { irq_register(MPM_IPC_INTID, mpm_ipc_irq, 0); s_ipc_armed = 1; }
    gic_set_edge(MPM_IPC_INTID, 1);
    gic_enable_irq(MPM_IPC_INTID, 0xA0u);
}
void mpm_ipc_irq_disarm(void)
{
    gic_disable_irq(MPM_IPC_INTID);
}

uint32_t mpm_status_word(unsigned idx) { return mmio_read(MPM_REG(MPM_STATUS, idx)); }
void     mpm_status_clear(void) { for (unsigned s = 0; s < MPM_REG_WIDTH; s++) mmio_write(MPM_REG(MPM_STATUS, s), 0u); }

#endif /* PLAT_BOARD_FOSSIL_GEN6 || PLAT_SOC_MSM8909 */
