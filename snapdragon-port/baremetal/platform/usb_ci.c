...
/* LOG CONSOLE POLICY (2026-09-08). The first connect used to replay the WHOLE
 * ring from boot (cursor 0), and a wedged bulk-IN transfer was waited on
 * forever -- the "log just stops while connected" the user keeps hitting.
 * Now: a fresh connection starts USB_LOG_TAIL bytes back (the recent past
 * only); the host asks for more by sending single characters on the CDC
 * port (echo -n F > /dev/ttyACM0):
 *   F  full replay of the ring from its oldest byte
 *   P  the previous boot's last 8 KB (ramlog_prev_tail)
 *   T  the last 4 KB again
 *   S  status line (ring size, cursor, stuck-transfer recoveries)
 * and a bulk-IN transfer that stays ACTIVE for USB_IN_STUCK_MS while the port
 * is configured is flushed and re-primed with the same bytes. */
#ifndef USB_LOG_TAIL
...
#if defined(USB_LOG_V2)
                if (!s_have_cursor) {            /* first connect of this boot: recent tail only */
                    uint32_t w = ramlog_written();
                    s_log_cursor = (w > USB_LOG_TAIL) ? w - USB_LOG_TAIL : 0u;
                    s_have_cursor = 1;
                }
                s_in_busy = 0;                   /* anything primed before the (re)configure is gone */
#endif
...
int usb_dev_init(void)
{
    ...
    /* FORCED DISCONNECT (2026-09-06). Since PS_HOLD resets became WARM the
     * controller and PHY survive a reboot still RUNNING, D+ pull-up up, so the
     * host believes the old session is alive while we re-initialise underneath
     * it: Windows then parks the device in "error state" and no replug helps,
     * because the watch never actually drops the line. Stop the controller
...
/* v193: after a SYSTEM collapse (RPM sleep set applied) the controller and
 * PHY come back in an unknown state and the host never sees us again. Drop
 * the run bit (init holds RS=0 for 200 ms = a real disconnect to the host),
 * forget the enumeration and run the full bring-up again. */
int usb_dev_reinit(void)
{
    if (s_inited) mmio_write(OP(USBCMD), mmio_read(OP(USBCMD)) & ~USBCMD_RS);
    s_inited = 0; s_running = 0; s_configured = 0;
    return usb_dev_init();
}
...
/* Live VBUS sense from the link controller: OTGSC.BSV (B-session valid, bit
 * 11) mirrors the PHY's VBUS comparator, so it is true for a charger as well
 * as a host and independent of WHICH charger IC the board wires USBIN to (the
 * TicWatches route it to an external SMB231, not the PMIC). -1 if the
 * controller is not running (its clocks may be off; do not touch it). */
#define OTGSC           0x64u
#define OTGSC_BSV       (1u << 11)
int usb_vbus_valid(void)
{
    if (!s_running) return -1;
    return (mmio_read(OP(OTGSC)) & OTGSC_BSV) ? 1 : 0;
}


/* PHY low-power for a sleep with no cable (sleep_floor.c). PORTSC.PHCD stops
 * the PHY clocks (its 480 MHz PLL runs whenever the port is not suspended,
 * cable or not). Controller stopped first, as msm_otg does; on exit the PHY
 * is re-clocked and the controller restarted -- a host that appears later
 * resets the bus, which is the normal enumeration path anyway. */
void usb_phy_lowpower(int on)
{
    ...
    if (on) {
        mmio_write(OP(USBCMD), mmio_read(OP(USBCMD)) & ~USBCMD_RS);
        timer_delay_ms(2u);
        mmio_write(OP(PORTSC), mmio_read(OP(PORTSC)) | PORTSC_PHCD);
    } else {
        mmio_write(OP(PORTSC), mmio_read(OP(PORTSC)) & ~PORTSC_PHCD);
        timer_delay_ms(2u);
        mmio_write(OP(USBCMD), mmio_read(OP(USBCMD)) | USBCMD_RS);
...
/* USB AS A WAKE SOURCE (2026-09-08, -DUSB_IRQ_WAKE). The no-watchdog sleep is
 * one long park and nothing services the endpoint until a button wakes the
 * core, so a cable-attached console goes silent for the whole sleep. The
 * controller can raise its core interrupt (SPI 134 -> GIC 166 on the 8909w
 * DT "core_irq") on transfer completion, a host write, a bus reset. The
 * handler only MASKS the controller and counts; usb_poll() does the work and
 * re-arms. A collapsed or WFI'd core wakes on any enabled GIC line, so the
 * console stays alive across the sleep without a periodic cadence. */
#if defined(USB_IRQ_WAKE)
#define USB_CORE_IRQ 166u
volatile uint32_t g_usb_irq_n;
static int s_irq_armed;
static void usb_irq_handler(void *arg)
{
    (void)arg;
    mmio_write(OP(USBINTR), 0);            /* masked until usb_poll re-arms */
    g_usb_irq_n++;
}
void usb_irq_arm(int on)
{
    if (!s_running) return;
    if (on && !s_irq_armed) { irq_register(USB_CORE_IRQ, usb_irq_handler, 0); gic_enable_irq(USB_CORE_IRQ, 0xA0u); s_irq_armed = 1; }
    if (!on) s_irq_armed = 0;
    mmio_write(OP(USBINTR), on ? (USBSTS_UI | USBSTS_UEI | USBSTS_PCI | USBSTS_URI) : 0u);
}
#else
void usb_irq_arm(int on) { (void)on; }
#endif
...
void usb_poll(void)
{
...
#if !defined(USB_LOG_V2)
    /* v168 behaviour, byte for byte: v169's rework killed the console while
    ...
    (void)s_have_cursor; (void)s_in_len; (void)s_in_t0; (void)s_in_recover;
#if defined(USB_IRQ_WAKE)
    if (s_irq_armed) mmio_write(OP(USBINTR), USBSTS_UI | USBSTS_UEI | USBSTS_PCI | USBSTS_URI);
#endif
    return;
#else
    /* Host commands first (they may move the cursor). The OUT dTD's token
     * holds the bytes NOT transferred, so received = size - remaining. */
    dcache_inval(&s_td[EP_IDX(EP_BULK_OUT, 0)], sizeof s_td[0]);
    if (!(s_td[EP_IDX(EP_BULK_OUT, 0)].token & DTD_TOKEN_ACTIVE)) {
        uint32_t remain = (s_td[EP_IDX(EP_BULK_OUT, 0)].token >> 16) & 0x7FFFu;
        uint32_t got = (remain <= sizeof s_outbuf) ? sizeof s_outbuf - remain : 0u;
        mmio_write(OP(off_complete()), 1u << EP_BULK_OUT);
        dcache_inval(s_outbuf, sizeof s_outbuf);
        for (uint32_t i = 0; i < got; i++) {
            switch (s_outbuf[i]) {
            case 'F': case 'f': s_log_cursor = 0u; con_puts("usb: log: FULL replay\n"); break;
            case 'T': case 't': { uint32_t w = ramlog_written(); s_log_cursor = (w > 4096u) ? w - 4096u : 0u; break; }
            case 'P': case 'p': ramlog_prev_tail(8192u); break;
            case 'S': case 's':
                con_puts("usb: log: written "); con_putdec(ramlog_written()); con_puts(" cursor "); con_putdec(s_log_cursor);
                con_puts(" in_recover "); con_putdec(s_in_recover); con_puts(" busy "); con_putdec((uint32_t)s_in_busy); con_puts("\n");
                break;
            default: break;
            }
        }
    ...
    if (s_in_busy) {
        ...
        if (s_td[EP_IDX(EP_BULK_IN, 1)].token & DTD_TOKEN_ACTIVE) {
            /* Stuck? Flush the endpoint and offer the same bytes again. A host
             * that is merely not reading (cat closed) also lands here every
             * few seconds; the re-prime is harmless for it. */
            if ((uint32_t)(timer_ms() - s_in_t0) > USB_IN_STUCK_MS) {
                mmio_write(OP(off_flush()), 1u << (16 + EP_BULK_IN));
                { uint32_t t0 = timer_ms(); while ((mmio_read(OP(off_flush())) & (1u << (16 + EP_BULK_IN))) && (uint32_t)(timer_ms() - t0) < 5u) ; }
                mmio_write(OP(off_complete()), 1u << (16 + EP_BULK_IN));
                ep_prime(EP_IDX(EP_BULK_IN, 1), s_inbuf, s_in_len);
                s_in_t0 = timer_ms();
                s_in_recover++;
            }
            return;
    ...
    if (n) {
        ep_prime(EP_IDX(EP_BULK_IN, 1), s_inbuf, n);
        s_in_busy = 1; s_in_len = n; s_in_t0 = timer_ms();
    }
#if defined(USB_IRQ_WAKE)
    if (s_irq_armed) mmio_write(OP(USBINTR), USBSTS_UI | USBSTS_UEI | USBSTS_PCI | USBSTS_URI);
#endif
