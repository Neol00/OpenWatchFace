...
{
    /* 2026-09-08: PWR_SLEEP is a one-way door on this part -- after it the
     * controller no longer answers the TP_MODE write, so the Gen 4 woke with
     * a dead glass (user: "touch is broken when you wake the fossil gen 4").
     * The vendor driver resumes it with a hardware reset + re-probe, which
     * this port does not have. Leave the controller awake across a sleep;
     * the TLMM summary IRQ mask in suspend_msm.c already keeps its INT line
     * from waking the collapsed core. Idle draw of an awake Raydium is small. */
    (void)sleep;
    return 0;
}
...
/* Gen 4 / Gen 6 build the Raydium driver; the FocalTech INT latch does not apply. */
