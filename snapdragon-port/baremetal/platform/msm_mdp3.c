...
/* ASYNC halves of the flush (2026-09-06, CPU1 frame push). mdp3_kick() points
 * DMA_P at `buf` and starts it WITHOUT the cache clean and WITHOUT waiting;
 * the caller must have cleaned `buf` out of D-cache already. mdp3_wait()
 * retires it. Either core may call these: they touch MMIO and the arch timer
 * only. mdp3_flush() below stays the synchronous clean+kick+wait. */
void mdp3_kick(const void *buf)
{
...
int mdp3_wait(uint32_t timeout_ms) { return mdp3_wait_dma_done(timeout_ms); }
uint32_t mdp3_frame_bytes(void) { return s_stride * s_h; }
