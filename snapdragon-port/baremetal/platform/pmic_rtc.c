...
/* Boot-time ownership report for the 8909w watches (read-only: consults the
 * arbiter's ownership table and reads the alarm block, writes nothing). This
 * is the gate for deep sleep's timer wake: if EE0 owns the alarm we can arm it
 * over SPMI; if not, the MPM sleep-clock timer is the fallback. The counter
 * ownership is reported alongside for the record - the vendor kernel never
 * writes it either (qcom,qpnp-rtc-write = 0 in both DTs). */
void rtc_probe_report(void)
{
    uint32_t now = 0, match = 0; uint8_t ctrl = 0;
    int ctr_w = spmi_writable(RTC_SID, RTC_CTRL);
    int alm_w = spmi_writable(RTC_SID, ALARM_CTRL);
    con_puts("rtc: counter owner ");
    con_puts(ctr_w == 1 ? "EE0 (writable)" : ctr_w == 0 ? "OTHER EE (write would fault)" : "unmapped");
    con_puts(", alarm owner ");
    con_puts(alm_w == 1 ? "EE0 (ARMABLE)" : alm_w == 0 ? "OTHER EE (not armable, use MPM)" : "unmapped");
    if (rtc_read_epoch(&now) == 0) { con_puts(", now="); con_putdec(now); }
    if (rtc_alarm_read(&match, &ctrl) == 0) { con_puts(", alarm match="); con_putdec(match); con_puts(" ctrl="); con_puthex(ctrl); }
    con_puts("\n");
}

/* Arm the alarm `secs` from now (counter-relative: the counter is uptime,
 * not epoch, on these watches - probe 2026-09-06). Writes the 4-byte match
 * then sets ALARM_CTRL bit7, preserving the other bits. 0 ok. */
int rtc_alarm_arm(uint32_t secs)
{
    uint32_t now = 0; uint8_t ctrl = 0, v[4];
    if (rtc_read_epoch(&now) < 0) return -1;
    uint32_t m = now + secs;
    v[0] = (uint8_t)m; v[1] = (uint8_t)(m >> 8); v[2] = (uint8_t)(m >> 16); v[3] = (uint8_t)(m >> 24);
    if (spmi_write(RTC_SID, ALARM_RW_BASE, v, 4) < 0) return -1;
    if (spmi_read8(RTC_SID, ALARM_CTRL, &ctrl) < 0) return -1;
    return spmi_write8(RTC_SID, ALARM_CTRL, (uint8_t)(ctrl | ALARM_CTRL_EN));
}
...
/* SELF-TEST (2026-09-06): prove the whole timer-wake chain awake, before any
 * sleep code depends on it. Arm for RTC_TEST_S, then poll the interrupt
 * counter; report fired-after or did-not-fire. */
extern volatile uint32_t g_pmic_irq_alarm;
#define RTC_TEST_S 10u
static uint32_t s_test_t0, s_test_n0; static int s_test_state;   /* 0 idle 1 armed 2 done */
void rtc_alarm_test_start(void)
{
    s_test_n0 = g_pmic_irq_alarm; s_test_t0 = timer_ms();
    int rc = rtc_alarm_arm(RTC_TEST_S);
    con_puts("rtc: alarm self-test armed for "); con_putdec(RTC_TEST_S); con_puts(" s, rc="); con_putdec((uint32_t)rc); con_puts("\n");
    s_test_state = (rc == 0) ? 1 : 2;
}
void rtc_alarm_test_poll(void)
{
    if (s_test_state != 1) return;
    uint32_t dt = timer_ms() - s_test_t0;
    if (g_pmic_irq_alarm != s_test_n0) {
        con_puts("rtc: ALARM FIRED after "); con_putdec(dt); con_puts(" ms - timer wake chain WORKS\n");
        rtc_alarm_disarm(); s_test_state = 2;
    } else if (dt > (RTC_TEST_S + 5u) * 1000u) {
        uint32_t m = 0, now = 0; uint8_t c = 0; rtc_alarm_read(&m, &c); rtc_read_epoch(&now);
        con_puts("rtc: alarm did NOT fire in "); con_putdec(dt); con_puts(" ms (match="); con_putdec(m);
        con_puts(" now="); con_putdec(now); con_puts(" ctrl="); con_puthex(c); con_puts(")\n");
        rtc_alarm_disarm(); s_test_state = 2;
    }
}
