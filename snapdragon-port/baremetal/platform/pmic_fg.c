...
#define CHG_PRES_REG        0x1008u     /* CHG_OPTION: "charger fitted" fuse, NOT vbus (diag only) */
#define CHG_PRESENT_BIT     (1u << 7)
/* qpnp-linear-charger (LBC): real-time interrupt status of the two charger
 * peripherals. From this watch's DTB: usb-chgpth@1300 "usbin-valid" = irq 1,
 * chgr@1000 "chg-done" = irq 7, "fast-chg-on" = irq 5. INT_RT_STS (+0x10)
 * mirrors the live level of each irq line, which is how the vendor driver's
 * qpnp_lbc_is_usb_chg_plugged_in() reads VBUS presence. 0x1008 above is
 * CHG_OPTION (bit 7 = PMIC charger present), which was being read as VBUS
 * and is why the watch never knew it was on a cable (2026-09-03). */
#define LBC_USB_INT_RT_STS  0x1310u
#define LBC_USBIN_VALID_BIT (1u << 1)
/* Bit 1 only, per the DTB's irq numbering. A previous revision also accepted
 * bit 2, on the strength of a single DOCKED sample reading 0x04 — with no
 * undocked sample to compare against. Bit 2 turns out to be set on battery
 * too, so that made the watch report USB permanently (C2 regression,
 * 2026-09-06). One sample of a signal is not evidence that it changes: what
 * identifies the right bit is the DIFFERENCE between plugged and unplugged,
 * which is what usbin-prev in the log line below now captures. */
#define LBC_USB_PRESENT_MASK  (1u << 1)
#define LBC_CHGR_INT_RT_STS 0x1010u
#define LBC_FAST_CHG_ON_BIT (1u << 5)
#define LBC_CHG_DONE_BIT    (1u << 7)
...
int fg_batt_ma(void)
{
    ...
    return smb231_batt_ma();      /* STC3117 coulomb counter, when its engine runs */
#else
    return -32768;
#endif
}
...
/* VBUS present? usbin-valid from the LBC, or the USB controller already
 * enumerated by a host (a charger never enumerates, a PC always does — the
 * second source covers a PMIC read that fails). */
int chg_usb_present(void)
{
    static int last = -2;               /* print only on change: this is polled at 2 Hz */
    uint8_t v = 0;
    int r, lbc = -1, bsv = usb_vbus_valid();
    /* Source order (S2 2026-09-04): the REAL C2/S2 device trees enable the
     * external SMB231 charger and the STC3117 gauge; VBUS goes to the SMB231
     * and the PMIC's own usbin-valid never asserts (it read 0 with a cable
     * in). The USB PHY's session-valid comparator sees VBUS whatever the
     * charger wiring, so it is the primary source; the LBC bit is kept as the
     * fallback (Gen 4, whose DT routes USBIN to the PMIC) and for the log.
     * usb_is_configured() is last: it is never cleared on a cable pull. */
    if (spmi_read8(0, LBC_USB_INT_RT_STS, &v) == 0) lbc = (v & LBC_USB_PRESENT_MASK) ? 1 : 0;
    /* THE PMIC IS THE VBUS SENSE ON THIS SoC, NOT THE PHY (2026-09-06).
     * The 8909 kernel runs msm_otg with otg_control = OTG_PMIC_CONTROL: VBUS
     * is not routed to the PHY's session-valid comparator at all, so OTGSC.BSV
     * reads 0 on a watch that is plainly charging. It was the PRIMARY source
     * here, which is why the C2 reported "USB power: no" on its dock, and why
     * usb_phy_force_vbus_valid() being a no-op (usb_phy_msm.c, written for the
     * Gen 6's QUSB2 PHY) never mattered: forcing session-valid is for device
     * mode, it is not a sense. So the LBC bit leads and BSV is only a
     * fallback for a PMIC read that failed. */
    int smb = -1;
#if defined(PLAT_CHG_SMB231)
    /* THE CHARGER IS EXTERNAL ON THE C2/S2 (2026-09-06): VBUS goes to the
     * SMB231 on QUP4, and neither the PMIC LBC nor the PHY ever sees it.
     * Its USBIN under/over-voltage status is the only honest source here. */
    smb = smb231_usb_present();
#endif
    if (smb >= 0)       r = smb;
    else if (lbc >= 0)  r = lbc;
    else if (bsv >= 0)  r = bsv;
    else                r = usb_is_configured() ? 1 : -1;
    /* Print on a change of the DECISION or of the raw byte. With the mask back
     * to bit 1 the decision may never change on this watch, and a diagnostic
     * that only fires on the thing that is broken tells us nothing. */
    { static uint32_t raw_last = 0x100u;
      int raw_changed = (raw_last != (uint32_t)v);
      raw_last = (uint32_t)v;
      if (r != last || raw_changed) {
        last = r;
        con_puts("chg: usb present="); con_putdec((uint32_t)(r < 0 ? 9 : r));
        con_puts(" (phy-bsv="); con_putdec((uint32_t)(bsv < 0 ? 9 : bsv));
        con_puts(" lbc-usbin="); con_putdec((uint32_t)(lbc < 0 ? 9 : lbc));
        con_puts(" enum="); con_putdec((uint32_t)usb_is_configured());
#if defined(PLAT_CHG_SMB231)
        con_puts(" "); smb231_log_state();
#endif
        /* Both sides of the transition on ONE line. The C2 runs its log over
         * the same USB it is trying to sense, so a line printed while
         * unplugged cannot be read live; carrying the previous sample forward
         * means the reconnect line shows the unplugged byte next to the
         * plugged one, and whichever bit differs is the real VBUS bit. */
        { static uint32_t prev = 0x100u;
          con_puts(" usbin-raw="); con_puthex((uint32_t)v);
          con_puts(" usbin-prev=");
          if (prev > 0xFFu) con_puts("--"); else con_puthex(prev);
          prev = (uint32_t)v; }
        con_puts(")\n");
      }
    }
...
/* Charging = VBUS present and the charger has not signalled end-of-charge.
 * (fast-chg-on alone would miss trickle/pre-charge on a deep-drained cell.) */
int chg_charging(void)
{
    uint8_t v = 0;
...
#if defined(PLAT_CHG_SMB231)
    /* SMB231 STATUS_B charge-type field: the charger itself saying whether it
     * is in pre/fast/taper charge. Only fall through to the LBC guesswork
     * when the part did not answer. */
    { int c = smb231_charging(); if (c >= 0) return c; }
#endif
    if (spmi_read8(0, LBC_CHGR_INT_RT_STS, &v) < 0) return -1;

    /* POSITIVE EVIDENCE FIRST (2026-09-06). This used to be "VBUS present and
     * CHG_DONE clear", i.e. charging was inferred from the ABSENCE of a
     * done bit. On a board where the PM8916 LBC is not the live charger that
     * bit never sets, so the watch reported "charging" forever — including at
     * 100% and including when the readout was simply wrong. fast-chg-on is
     * the charger actually saying it is pushing current, so trust it when it
     * is set; only fall back to the old inference when it is not. */
    if (v & LBC_FAST_CHG_ON_BIT) return 1;
    if (v & LBC_CHG_DONE_BIT)    return 0;
    /* Neither bit set with VBUS in: either trickle/pre-charge (real, and the
     * reason this is not simply "no"), or an LBC that is not wired on this
     * watch. The raw byte is in the chg: log line so the two can be told
     * apart on hardware. */
    return 1;
}
