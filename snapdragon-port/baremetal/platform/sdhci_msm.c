...
int emmc_init(void)
{
    ...
    /* A board that has not declared its eMMC storage parameters (board
     * header: PLAT_HAVE_EMMC_STORAGE + PLAT_SDC1_PAD_* + PLAT_EMMC_VDD_*)
     * gets no controller access at all -- the pre-2026-08-30 behaviour for
     * everything but the Gen 6. */
    con_puts("emmc: board declares no eMMC storage\n");
    s_emmc_ok = 0;
    return -1;
#else
    ...
    /* STORAGE30 — SDC1 PAD CONTROL (TLMM SDC1_HDRV_PULL_CTL, the same
     * TLMM+0x10A000 register on sdm429w AND msm8909; field layout: drive
     * data[2:0] cmd[5:3] clk[8:6], pull data[10:9] cmd[12:11] clk[14:13]
     * rclk[15]). The ACTIVE-state value is per board -- straight from each
     * watch's DTB sdc1_*_on pinctrl nodes -- in PLAT_SDC1_PAD_VAL, with
     * PLAT_SDC1_PAD_MASK naming the bits that board actually owns (the
     * 8909 has no sdc1_rclk pin, so its mask leaves bit 15 alone). If the
     * pads sat in a parked/sleep state (weak drive, pulls off), open-drain
     * ident responses corrupt (the CMD2 CRC) and marginal edges eat later
     * responses (the CMD7 silence). */
    {
        uint32_t pad_old = mmio_read(PLAT_SDC1_PAD_CTL);
        mmio_write(PLAT_SDC1_PAD_CTL,
                   (pad_old & ~(uint32_t)PLAT_SDC1_PAD_MASK) |
                   ((uint32_t)PLAT_SDC1_PAD_VAL & (uint32_t)PLAT_SDC1_PAD_MASK));
        bdiag_puts("emmc: sdc1 pads "); bdiag_puthex(pad_old);
        bdiag_puts(" -> "); bdiag_puthex(mmio_read(PLAT_SDC1_PAD_CTL)); bdiag_puts("\n");
    }
    __asm__ volatile("dsb sy" ::: "memory");
...
#if defined(PLAT_SDHC_HAS_CQE) && PLAT_SDHC_HAS_CQE
    {
...
static int emmc_full_init(void)
{
     ...
     * CMD1: the engine works, the CARD is electrically absent. The card's
     * VCC LDO is per board (Gen 6: PM660 L19; Gen 4 / TicWatch C2: PM8916
     * L8 -- both from the DTB sdhc1 vdd-supply phandle), named by
     * PLAT_EMMC_VDD_SID / PLAT_EMMC_VDD_EN_CTL (qpnp LDO base + 0x46).
     * Read EN_CTL bit7; if the handoff left it OFF, attempt the enable --
     * a calculated risk: voltage config untouched, only the enable bit.
     * aboot just read this image off the card, so the rail is normally
     * already on and this is a read + a log line. */
    {
        uint8_t en = 0;
        if (spmi_read8(PLAT_EMMC_VDD_SID, PLAT_EMMC_VDD_EN_CTL, &en) == 0) {
            bdiag_puts("emmc: VDD EN_CTL="); bdiag_puthex(en); bdiag_puts("\n");
            if (!(en & 0x80u)) {
                spmi_write8(PLAT_EMMC_VDD_SID, PLAT_EMMC_VDD_EN_CTL, (uint8_t)(en | 0x80u));
                timer_delay_ms(10);           /* LDO soft-start */
                bdiag_puts("emmc: VDD enable attempted\n");
            }
        }
    }
#endif

    /* STORAGE21: NO VDD power-cycle EVER — on the Gen 6 the display shares
     * that rail (STORAGE20 went black). Two attempts differ only in the reset spell:
...
/* ONE LOCK FOR THE CONTROLLER (2026-09-03). The radio bring-up runs in the
 * app's net task and streams 5 MB of firmware off the modem partition while
 * the UI task keeps writing the log and blackbox: two tasks in the SDHCI
 * command path at once corrupted a read ("cannot read WCNSS B06"). Every
 * block transfer takes this recursive mutex; before the scheduler runs
 * (early storage init) there is nothing to contend with and no lock. */
#include "FreeRTOS.h"
#include "semphr.h"
#include "task.h"
static SemaphoreHandle_t s_emmc_lock;
static void emmc_lock(void)
{
    if (xTaskGetSchedulerState() == taskSCHEDULER_NOT_STARTED) return;
    if (!s_emmc_lock) s_emmc_lock = xSemaphoreCreateRecursiveMutex();
    if (s_emmc_lock) xSemaphoreTakeRecursive(s_emmc_lock, portMAX_DELAY);
}
static void emmc_unlock(void)
{
    if (xTaskGetSchedulerState() == taskSCHEDULER_NOT_STARTED) return;
...
{
    int r; emmc_lock(); r = emmc_read_block_unlocked(lba, dst); emmc_unlock(); return r;
}
static int emmc_read_block_unlocked(uint32_t lba, void *dst)
{
...
/* The ONE exception to "boot chain partitions are unwritable": the over-the-
 * air installer (bootimg_write.c) replaces our own image in `boot`. It gets a
 * separate window with the same once-per-boot rule, armed only by that file
 * after the downloaded image has been verified, and it never covers anything
 * but the `boot` partition it was armed with. aboot/sbl1/tz/rpm/modem/persist
...
int emmc_write_window_boot(uint32_t lba, uint32_t nblocks)
{
    if (s_bwin_state != 0u || nblocks == 0u) {
        s_bwin_state = 2u;
        bdiag_puts("emmc: boot write window LOCKED OUT\n");
        return -1;
    }
    s_bwin_lba = lba; s_bwin_n = nblocks; s_bwin_state = 1u;
    bdiag_puts("emmc: boot write window armed @"); bdiag_puthex(lba);
...
static int emmc_write_block_unlocked(uint32_t lba, const void *src);
int emmc_write_block(uint32_t lba, const void *src)
{
    int r; emmc_lock(); r = emmc_write_block_unlocked(lba, src); emmc_unlock(); return r;
}
static int emmc_write_block_unlocked(uint32_t lba, const void *src)
{
