...
/* qpnp-rtc ALARM peripheral (sid 0, base 0x6100). DT: interrupts = <0 0x61 1>
 * -> peripheral 0x61, irq index 1. Same QPNP INT register layout as PON. */
#define ALM_SID             0u
#define ALM_BASE            0x6100u
#define ALM_INT_SET_TYPE    (ALM_BASE + 0x11u)
#define ALM_INT_LATCHED_CLR (ALM_BASE + 0x14u)
#define ALM_INT_EN_SET      (ALM_BASE + 0x15u)
#define ALM_IRQ_BIT         (1u << 1)
static int s_alm_apid = -1;
volatile uint32_t g_pmic_irq_alarm;    /* RTC alarm interrupts seen */

/* Census — suspend_msm.c prints these so a dead chain is visible. */
volatile uint32_t g_pmic_irq_armed;    /* chain up: button polls may gate on the edge counters */
...
static void pmic_irq_handler(void *arg)
{
    ...
    for (unsigned w = 0; w < PIC_ACC_WORDS; w++) {
        ...
        while (acc) {
            ...
            } else if ((int)apid == s_alm_apid) {
                g_pmic_irq_alarm++;
                g_pmic_irq_wake = 1u;
                spmi_write8(ALM_SID, ALM_INT_LATCHED_CLR, (uint8_t)st);
            }
...
/* Route the RTC alarm's interrupt (deep sleep's timer wake). Call after
 * pmic_irq_init() so the GIC line is registered. 0 ok, -1 not mapped. */
int pmic_irq_alarm_init(void)
{
    s_alm_apid = spmi_apid_of(ALM_SID, ALM_BASE);
    if (s_alm_apid < 0) { con_puts("pmic-irq: alarm periph NOT MAPPED\n"); return -1; }
    uint8_t t[3] = { 0, 0, 0 };
    if (spmi_read(ALM_SID, ALM_INT_SET_TYPE, t, 3) < 0) return -1;
    t[0] |=  ALM_IRQ_BIT;    /* edge */
    t[1] |=  ALM_IRQ_BIT;    /* rising */
    t[2] &= ~ALM_IRQ_BIT;
    spmi_write(ALM_SID, ALM_INT_SET_TYPE, t, 3);
    mmio_write(PIC_ACC_ENABLE(s_alm_apid), PIC_ACC_ENABLE_BIT);
    spmi_write8(ALM_SID, ALM_INT_LATCHED_CLR, ALM_IRQ_BIT);
    spmi_write8(ALM_SID, ALM_INT_EN_SET,      ALM_IRQ_BIT);
    con_puts("pmic-irq: rtc alarm apid="); con_putdec((uint32_t)s_alm_apid); con_puts(" armed on the PON chain\n");
    return 0;
}
