...
/* SLEEP: park the cluster clock on the crystal (mux 0, div 1 = 19.2 MHz).
 * CPU0 is about to be power-collapsed, but CPU1 and the L2 stay clocked from
 * this RCG for the whole sleep; 200 MHz of clock tree into an idle L2 is
 * dynamic power for nothing. Voltage is left alone (lower clock at the same
 * rail is always safe). exit() restores the exact CFG word. */
static uint32_t s_sleep_cfg; static int s_sleep_parked;
int cpu_clk_sleep_enter(void)
{
    if (s_sleep_parked) return 0;
    if (mmio_read(A7_CMD_RCGR) & CMD_DIRTY_MASK) return -1;
    s_sleep_cfg = mmio_read(A7_CFG_RCGR);
    mmio_write(A7_CFG_RCGR, s_sleep_cfg & ~(CFG_DIV_MASK | CFG_SRC_MASK));   /* src 0 = XO, div 1 */
    __asm__ volatile("dsb sy" ::: "memory");
    if (a7_rcg_update() < 0) { mmio_write(A7_CFG_RCGR, s_sleep_cfg); (void)a7_rcg_update(); return -1; }
    s_sleep_parked = 1;
    return 0;
}
...
{
    if (!s_sleep_parked) return 0;
    mmio_write(A7_CFG_RCGR, s_sleep_cfg);
    __asm__ volatile("dsb sy" ::: "memory");
    s_sleep_parked = 0;
    return a7_rcg_update();
}
