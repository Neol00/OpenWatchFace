...
/* SLEEP (2026-09-07): with the panel in DCS sleep-in nothing moves through
 * MDSS, but every branch clock and the mdp root kept running. Gate them; the
 * GDSC and the DSI PLL stay up so every register keeps its value and no
 * re-init is needed on wake -- wake is just the enables again. */
static uint32_t s_sleep_mask;
static const uint32_t k_mdss_branches[] = {
    GCC_MDSS_PCLK0_CBCR, GCC_MDSS_BYTE0_CBCR, GCC_MDSS_ESC0_CBCR,
    GCC_MDSS_VSYNC_CBCR, GCC_MDSS_MDP_CBCR, GCC_MDSS_AXI_CBCR, GCC_MDSS_AHB_CBCR };
void gcc_mdss_sleep(int on)
{
    unsigned n = sizeof k_mdss_branches / sizeof k_mdss_branches[0];
    if (on) {
        s_sleep_mask = 0;
        for (unsigned i = 0; i < n; i++) {
            uint32_t off = k_mdss_branches[i];
            if (GCC_R(off) & CBCR_CLK_ENABLE) { s_sleep_mask |= 1u << i; GCC_W(off, GCC_R(off) & ~CBCR_CLK_ENABLE); }
        }
        __asm__ volatile("dsb sy" ::: "memory");
    } else {
        for (unsigned i = n; i-- > 0;)          /* AHB first, link clocks last */
            if (s_sleep_mask & (1u << i)) (void)branch_enable(k_mdss_branches[i]);
        timer_delay_us(200u);
        s_sleep_mask = 0;
    }
}
