...
static volatile uint32_t s_touch_evt;   /* set by the gpio13 edge interrupt */
volatile uint32_t g_touch_irq_armed, g_touch_irq_n;
static void touch_irq(void *a) { (void)a; s_touch_evt = 1u; g_touch_irq_n++; }
/* Arm the INT-pin latch. Called from the 8909 one-shot once the GIC path is
 * known good; a failure leaves the always-read behaviour in place. */
void touch_ft_arm_irq(void)
{
#if defined(PLAT_SOC_MSM8909)
    extern int tlmm_irq_enable(uint32_t pin, uint32_t detect, void (*fn)(void *), void *arg);
    tlmm_cfg(PLAT_TOUCH_IRQ_GPIO, 0u, 3u, 2u, 0);          /* input, pull-up */
    if (tlmm_irq_enable(PLAT_TOUCH_IRQ_GPIO, 3u, touch_irq, 0) == 0) { s_touch_evt = 1u; g_touch_irq_armed = 1u; }
#endif
}
...
int touch_read(uint16_t *x, uint16_t *y)
{
    ...
    /* INT LATCH (2026-09-04, idle-load work). The pin is edge-triggered, so a
     * poll cannot sample it — but a TLMM edge interrupt CAN catch it. Once
     * tlmm_irq.c has armed gpio13, the bus is read only after an edge was
     * latched or while a finger is known to be down (the release is then
     * seen by the read itself). Before arming, the unconditional read below
     * is exactly what shipped and measured 400 reads / 600 ms per 10 s. */
    if (g_touch_irq_armed && !s_touch_evt && !s_last_down) return 0;
    s_touch_evt = 0;

    /* (Historical note) DELIBERATELY NOT INT-GATED. The Gen 6's Raydium declares its interrupt
