...
/* Make a PS_HOLD drop (APPS watchdog bite, hard hang recovery) a WARM reset:
 * the SoC rails stay up, so DDR and IMEM keep their contents through it. The
 * PMIC's default here is HARD (PON_REASON1 = HARD_RESET after the first
 * failed power-collapse resume, 2026-09-06), which wiped every breadcrumb.
 * The vendor kernel runs with WARM in normal operation for the same reason. */
/* Undo pon_ps_hold_warm(): back to the PMIC's HARD reset on PS_HOLD. The warm
 * setting persists in the PMIC across reboots and flashes, and USB device
 * enumeration failed on every boot after it was set (2026-09-06). */
/* Two breadcrumb bits that survive a PMIC HARD reset (2026-09-07): the
 * SOFT_RB_SPARE register keeps its value across PS_HOLD resets (that is how
 * aboot learns "reboot to fastboot"); its restart-reason field sits above
 * bit 1 on this part (pon_reason_shift), so [1:0] are ours. */
static int pon_crumb_ok(void)
{   /* only when the restart-reason field starts at bit 2: a crumb must never
     * read as PON_RESTART_REASON_BOOTLOADER to aboot */
    uint8_t st = 0; if (spmi_read8(0, PON_PERPH_SUBTYPE, &st) < 0) return 0;
    return pon_reason_shift(st) == 2u;
}
/* v177: SOFT_RB_SPARE is cleared by aboot at every boot (v158), so the crumb
 * moved to DVDD_RB_SPARE (PON base + 0x8D): a PMIC scratch byte in the DVDD
 * domain that neither aboot nor a PS_HOLD reset touches. Whole byte is ours. */
#define PON_DVDD_RB_SPARE      (PON_BASE + 0x8Du)
void pon_crumb_write(uint8_t v) { (void)pon_crumb_ok; (void)spmi_write8(0, PON_DVDD_RB_SPARE, v); }
uint8_t pon_crumb_read(void) { uint8_t v = 0xFF; spmi_read8(0, PON_DVDD_RB_SPARE, &v); return v; }
void pon_crumb_report(void)
{
    uint8_t v = pon_crumb_read();
...
{
    (void)pon_reset_config(PON_POWER_OFF_HARD_RESET);
    uint8_t rb = 0xFF; spmi_read8(0, PON_PS_HOLD_RST_CTL, &rb);
    con_puts("pon: PS_HOLD reset type -> HARD (restored); readback ctl="); con_puthex(rb); con_puts("\n");
}

void pon_ps_hold_warm(void)
{
    (void)pon_reset_config(PON_POWER_OFF_WARM_RESET);
    uint8_t rb = 0xFF, en = 0xFF;
    spmi_read8(0, PON_PS_HOLD_RST_CTL, &rb); spmi_read8(0, PON_PS_HOLD_RST_CTL2, &en);
    con_puts("pon: PS_HOLD reset type -> WARM; readback ctl="); con_puthex(rb);
    con_puts(" ctl2="); con_puthex(en); con_puts(" (type bits should read 1)\n");
}
...
/* Real power-off: drivers/power/reset/msm-poweroff.c do_msm_poweroff() is
 * qpnp_pon_system_pwr_off(PON_POWER_OFF_SHUTDOWN) followed by the same
 * PS_HOLD drop a reboot uses. The PMIC then executes a SHUTDOWN (type 0x04)
 * instead of a reset: every rail off, the PMIC itself stays on its sleep
 * clock waiting for a PON trigger. Both PM8916 and PM660 wake on the power
 * key (KPDPWR) or on USB insertion (USB_CHG) by their default PON
 * configuration, which is what puts the user back at aboot. */
#define PON_POWER_OFF_SHUTDOWN   0x04u
void poweroff_now(void)
{
    con_puts("reboot: POWER OFF (PMIC shutdown)\n");
    (void)pon_reset_config(PON_POWER_OFF_SHUTDOWN);
    mmio_write(MSM_IMEM_RESTART_REASON, 0u);        /* no reboot reason for aboot */
    dsb();
    timer_delay_ms(2);                               /* let the PON writes latch (slow clock domain) */
    mmio_write(MSM_PSHOLD_BASE, 0u);
    dsb();
    for (;;) { __asm__ volatile("wfi"); }
}
...
static void deadman_expired(TimerHandle_t t)
{
    ...
    {   /* v197: leave the same record the CPU faults leave (class 8) */
        extern char __ramlog_end[];
        volatile uint32_t *f = (volatile uint32_t *)(__ramlog_end - 32);
        f[0] = 0xFA000008u; f[1] = 0; f[2] = 0; f[3] = 0x6e6d6464u;
