...
# partition (../dtbs/darter-stock.dtb, byte-identical to the copy in its
...
#
# Usage: ./mk-bootimg-gen5.sh [payload.bin] [dtb]  ->  build/darter/owf-boot.img
set -e
cd "$(dirname "$0")/.."

mkdir -p "build/darter"

PAYLOAD="${1:-build/darter/owf.bin}"
DTB="${2:-${DTB:-../dtbs/darter-stock.dtb}}"
OUT="build/darter/owf-boot.img"
...
KERNEL="build/darter/owf-with-dtb.bin"
...
if command -v fdtget >/dev/null 2>&1; then
  ...
  case "$bid" in
    ...
    *)  echo "REFUSING: $DTB has board-id '$bid', expected '8 113' (darter)"; exit 1 ;;
  esac
fi

printf '' | gzip > build/darter/empty-ramdisk.gz

python3 tools/mkbootimg_v0.py \
  --kernel "$KERNEL" \
  --ramdisk build/darter/empty-ramdisk.gz \
  --pagesize 2048 \
  --base 0x80000000 \
