...
# partition (../dtbs/triggerfish-stock.dtb, byte-identical to the copy in its
...
#
# Usage: ./mk-bootimg-gen5.sh [payload.bin] [dtb]  ->  build/gen5/owf-boot.img
set -e
cd "$(dirname "$0")/.."

mkdir -p "build/gen5"

PAYLOAD="${1:-build/gen5/owf.bin}"
DTB="${2:-${DTB:-../dtbs/triggerfish-stock.dtb}}"
OUT="build/gen5/owf-boot.img"
...
KERNEL="build/gen5/owf-with-dtb.bin"
...
if command -v fdtget >/dev/null 2>&1; then
  ...
  case "$bid" in
    ...
    *)  echo "REFUSING: $DTB has board-id '$bid', expected '8 15' (triggerfish)"; exit 1 ;;
  esac
fi

printf '' | gzip > build/gen5/empty-ramdisk.gz

python3 tools/mkbootimg_v0.py \
  --kernel "$KERNEL" \
  --ramdisk build/gen5/empty-ramdisk.gz \
  --pagesize 2048 \
  --base 0x80000000 \
