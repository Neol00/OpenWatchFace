#!/bin/sh
# mk-bootimg-s2.sh — pack the c2 payload into an Android boot image that the
# TicWatch S2's aboot will load.
#
...
if [ -n "$DTB" ]; then
  ...
  KERNEL="build/s2/owf-with-dtb.bin"
...
printf '' | gzip > build/s2/empty-ramdisk.gz
...
python tools/mkbootimg_v0.py \
  --kernel "$KERNEL" \
  --ramdisk build/s2/empty-ramdisk.gz \
  --pagesize 2048 \
  --base 0x80000000 \
