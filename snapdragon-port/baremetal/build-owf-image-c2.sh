...
# C2 boot payload: snapdragon-port runtime + compat layer + app TU + LVGL.
#
...
# builds a working image; pick an explicit OWF_C2_FW if set, else the flat
# legacy directory, else the first per-watch one, and SAY which was used.
nv_dir_c2() {
  if [ -n "${OWF_C2_FW:-}" ]; then echo "$OWF_C2_FW"; return; fi
  if [ -f ../firmware/c2/wcnss_nv.c ]; then echo ../firmware/c2; return; fi
  for d in ../firmware/c2-*; do
...
COMMON="-mcpu=cortex-a7 -marm -mfpu=neon-vfpv4 -mfloat-abi=hard -O2 -g \
  -DPLAT_BOARD_TICWATCH_C2 -DLV_CONF_INCLUDE_SIMPLE -DUSE_CPU_PC_8909 ${CFLAGS_EXTRA:-}"
...
CXXFLAGS="$COMMON -std=gnu++17 -fno-exceptions -fno-rtti -fno-use-cxa-atexit \
  --specs=nosys.specs -DBOARD_SELECT=BOARD_ID_TICWATCH_C2 \
...
if [ ! -f ../firmware/c2/wcnss_nv.c ] && [ -f "$(nv_dir_gen4)/wcnss_nv.c" ]; then
  mkdir -p ../firmware/c2
  sed -e 's/^const uint8_t wcnss_mac\[6\] = .*/const uint8_t wcnss_mac[6] = { 0x9A, 0x29, 0xA6, 0xF1, 0xAC, 0xC2 };  \/* locally administered, derived for the C2 *\//' \
      -e '1i /* Borrowed for the TicWatch C2 from the Gen 4 blob (same WCN3620A); regenerated by build-owf-image-c2.sh. */' \
      "$(nv_dir_gen4)/wcnss_nv.c" > ../firmware/c2/wcnss_nv.c
  echo "[owf] firmware/c2/wcnss_nv.c generated from the Gen 4 NV blob"
fi
...
if [ -f "$NVC" ]; then cc_one "$NVC" "" "$B/wcnss_nv.o"; else echo "[owf] no wcnss_nv.c under firmware/c2* - NV download will be skipped"; fi
