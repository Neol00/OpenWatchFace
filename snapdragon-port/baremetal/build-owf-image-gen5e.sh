...
# fastboot serial (firmware/gen5e-C0F8482D2529/...), because persist -- and the
...
nv_dir_gen5() {
  if [ -n "${OWF_GEN5E_FW:-}" ]; then echo "$OWF_GEN5E_FW"; return; fi
  if [ -f ../firmware/gen5e/wcnss_nv.c ]; then echo ../firmware/gen5e; return; fi
  for d in ../firmware/gen5e-*; do
...
COMMON="-mcpu=cortex-a7 -marm -mfpu=neon-vfpv4 -mfloat-abi=hard -O2 -g \
  -DPLAT_BOARD_FOSSIL_GEN5 -DPLAT_BOARD_FOSSIL_GEN5E -DPLAT_NO_ROTARY_CROWN -DLV_CONF_INCLUDE_SIMPLE -DUSE_CPU_PC_8909 ${CFLAGS_EXTRA:-}"
...
CXXFLAGS="$COMMON -std=gnu++17 -fno-exceptions -fno-rtti -fno-use-cxa-atexit \
  --specs=nosys.specs -DBOARD_SELECT=BOARD_ID_FOSSIL_GEN5E \
...
echo "[owf] pack with: sh tools/mk-bootimg-gen5e.sh $B/owf.bin"
