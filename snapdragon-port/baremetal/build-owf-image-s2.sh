#!/bin/sh
# build-owf-image-s2.sh — link the REAL OpenWatchFace firmware into a TicWatch
# S2 (tunny) boot payload: snapdragon-port runtime + compat layer + app TU + LVGL.
#
# The S2 is the C2 with a 400x400 panel, so this is build-owf-image-c2.sh with
# BOTH board defines set (PLAT_BOARD_TICWATCH_C2 keeps every shared driver
# gate, PLAT_BOARD_TICWATCH_S2 selects boards/ticwatch_s2.h) and the NV blob
# taken from firmware/s2/ (pulled from the S2 itself 2026-09-04).
...
# builds a working image; pick an explicit OWF_S2_FW if set, else the flat
# legacy directory, else the first per-watch one, and SAY which was used.
nv_dir_s2() {
  if [ -n "${OWF_S2_FW:-}" ]; then echo "$OWF_S2_FW"; return; fi
  if [ -f ../firmware/s2/wcnss_nv.c ]; then echo ../firmware/s2; return; fi
  for d in ../firmware/s2-*; do
...
# set -e was missing here alone (2026-09-17): a failed LINK still ended in exit 0
# and the previous owf.bin got packed. The C2/Gen 4/Gen 6 scripts have it.
set -e
cd "$(dirname "$0")"
...
COMMON="-mcpu=cortex-a7 -marm -mfpu=neon-vfpv4 -mfloat-abi=hard -O2 -g \
  -DPLAT_BOARD_TICWATCH_C2 -DPLAT_BOARD_TICWATCH_S2 -DLV_CONF_INCLUDE_SIMPLE -DUSE_CPU_PC_8909 ${CFLAGS_EXTRA:-}"
...
CXXFLAGS="$COMMON -std=gnu++17 -fno-exceptions -fno-rtti -fno-use-cxa-atexit \
  --specs=nosys.specs -DBOARD_SELECT=BOARD_ID_TICWATCH_S2 \
...
# The C2 cannot be pulled (no root/TWRP), so when it has no blob of its own
# borrow the Gen 4's: same msm8909w + WCN3620A. The MAC is rewritten to a
# locally-administered one so the two watches never share an address.
if [ ! -f ../firmware/s2/wcnss_nv.c ] && [ -f "$(nv_dir_gen4)/wcnss_nv.c" ]; then
  mkdir -p ../firmware/s2
  sed -e 's/^const uint8_t wcnss_mac\[6\] = .*/const uint8_t wcnss_mac[6] = { 0x9A, 0x29, 0xA6, 0xF1, 0xAC, 0x52 };  \/* locally administered, derived for the S2 *\//' \
      -e '1i /* Borrowed for the TicWatch C2 from the Gen 4 blob (same WCN3620A); regenerated by build-owf-image-s2.sh. */' \
      "$(nv_dir_gen4)/wcnss_nv.c" > ../firmware/s2/wcnss_nv.c
...
if [ -f "$NVC" ]; then cc_one "$NVC" "" "$B/wcnss_nv.o"; else echo "[owf] no wcnss_nv.c under firmware/s2* - NV download will be skipped"; fi
...
echo "[owf] pack with: sh tools/mk-bootimg-s2.sh $B/owf.bin"
