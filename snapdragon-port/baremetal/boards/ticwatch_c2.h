 ...
 * FROM-DTB (skipjack.dts), and byte-identical to the Fossil Gen 4's tree:
 * pronto@a21b000 pas-id 6 with memory-region pheripheral_region@0
 * (0x8A100000 + 0x500000, no-map), wcnss-wlan gpios tlmm 40..44 with
 * wcnss_default = 6 mA pull-up, tz-log@8600720 (+0x1000, header in place),
 * rails 8916_s1_corner / l3_corner_ao / l7 / s3 / l9 / l5. See the Gen 4
 * header for the derivation; the values are the SoC's, not the vendor's. */
 ...
 * skipjack DTB -- byte-for-byte the same node as the Fossil Gen 4's. Declaring
...
#if !defined(NO_CPU_PC_8909)     /* build with -UUSE_CPU_PC_8909 -DNO_CPU_PC_8909 for the WFI-only path */
 ...
 * charge state come from the SMB231 over I2C (platform/chg_smb231.c); the
 * VM-BMS voltage path stays as the state-of-charge source. Pins from the DT:
 * smb_stat gpio49 (STAT, input), smb_susp gpio58 (output-low), gauge_alarm
 * gpio28 (unused). */
#define PLAT_CHG_SMB231        1
#define PLAT_SMB_I2C_SDA_GPIO  14u
#define PLAT_SMB_I2C_SCL_GPIO  15u
#define PLAT_SMB_STAT_GPIO     49u
#define PLAT_SMB_SUSP_GPIO     58u
