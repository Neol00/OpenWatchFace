 ...
 * IMPLEMENTED in arduino_glue.cpp, bound to the snapdragon-port runtime.
 */
#pragma once
...
#endif
    uint64_t getEfuseMac()       {                       /* ESP layout: octet 0 in the low byte */
        const uint8_t *m = wlan_mac(); uint64_t v = 0;
        for (int i = 5; i >= 0; i--) v = (v << 8) | m[i];
        return v;
    }
...
/* NTP: configTzTime applies the POSIX TZ and starts lwIP's SNTP client on
 * the WLAN link (platform/wlan_net.c); getLocalTime reads the clock back.
 * The app polls getLocalTime for up to 5 s after configTzTime, so the SNTP
 * round trip has to land in that window (it does: one UDP exchange). */
extern "C" void net_sntp_start(const char *s1, const char *s2);
extern "C" int  net_time_synced(void);
...
static inline void configTzTime(const char *tz, const char *s1, const char *s2 = nullptr, const char* = nullptr) {
    if (tz) { setenv("TZ", tz, 1); tzset(); }
    net_sntp_start(s1, s2);
}
static inline void configTime(long, int, const char *s1, const char *s2 = nullptr, const char* = nullptr) {
    net_sntp_start(s1, s2);
}
#endif
