 ...
 * ArduinoCore-API's Common.h) on top of the snapdragon-port bare-metal runtime.
 *
...
/* snapdragon-port runtime (platform.h is C) */
extern "C" {
...
extern "C" PinStatus digitalRead(pin_size_t pin)
{
    ...
    /* IDLE-LOAD FIX (2026-09-04): this is polled every loop pass, and each
     * poll was an SPMI transaction. Once pmic_irq.c has the PON interrupt
     * chain up, a press is an edge on its counter, so the bus is only read
     * when the counter moved or while the button is known to be held (the
     * release is then seen by the read itself). Before the chain is armed,
     * and on the Gen 6, this is the plain poll it always was. */
    extern volatile uint32_t g_pmic_irq_armed, g_pmic_irq_kpdpwr, g_pmic_irq_resin;
    if (pin == FOSSIL_PIN_KPDPWR) {
        static uint32_t seen; static bool held;
        if (g_pmic_irq_armed && g_pmic_irq_kpdpwr == seen && !held) return HIGH;
        seen = g_pmic_irq_kpdpwr;
        held = pon_kpdpwr_pressed() == 1;
        return held ? LOW : HIGH;
    }
    if (pin == FOSSIL_PIN_RESIN) {
        static uint32_t seen; static bool held;
        if (g_pmic_irq_armed && g_pmic_irq_resin == seen && !held) return HIGH;
        seen = g_pmic_irq_resin;
