/* owf_fossil_lvgl.h — snapdragon-port LVGL bring-up for the real OpenWatchFace UI.
 *
...
extern "C" {
    ...
    void     fb_flush_buf(const void *buf);   /* async push when CPU1 serves it */
    void    *fb_ptr2(void);                   /* second direct-mode buffer       */
...
static void owf_fossil_flush_cb(lv_display_t *disp, const lv_area_t *area, uint8_t *px_map) {
    if (area) {
    ...
    if (lv_display_flush_is_last(disp)) {
        ...
        /* px_map is the base of the buffer just rendered (direct mode). With
         * CPU1 up this returns once the push has STARTED; LVGL then renders
         * the next frame into the other buffer while the panel receives this one. */
        fb_flush_buf(px_map);
...
static inline bool owf_fossil_lvgl_begin(void) {
    ...
    lv_display_set_buffers(d, fb, fb_ptr2(), w * h * bpp, LV_DISPLAY_RENDER_MODE_DIRECT);
...
/* ---- suspend helpers for sleep_power.h's Fossil branch (2026-09-03) --------
 * Mirrors owf_tuya_suspend_sleep / owf_tuya_display_wake: block in the
 * platform suspend until a button or the armed deadline. A TIMER wake comes
 * back DARK (panel still off) so the caller can run its background check
 * without lighting the screen; returns true only on a button press. */
...
static inline bool owf_fossil_suspend_sleep(uint32_t wake_s) {
    if (wake_s) plat_suspend_set_timer_us((unsigned long long)wake_s * 1000000ULL);
    plat_suspend_keep_dark(1);
    plat_suspend();
    plat_suspend_keep_dark(0);
    return plat_suspend_last_cause() == 1;        /* PLAT_WAKE_BUTTON */
}
static inline void owf_fossil_display_wake(void) { plat_display_on(); }
