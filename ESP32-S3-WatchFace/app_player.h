...
static void app_open_player(void) {
  ...
  lv_obj_set_height(ctl, 84 + 24);          // 84px button + headroom so nothing clips
  ...
  lv_obj_set_flex_align(ctl, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER,
                        LV_FLEX_ALIGN_CENTER);
  lv_obj_set_style_pad_column(ctl, 22, 0);
  lv_obj_set_style_pad_top(ctl, 16, 0);

  pl_make_btn(ctl, LV_SYMBOL_PREV, PCMD_PREV, 66, 0x2A2A2A);
  pl_play_lbl = pl_make_btn(ctl, LV_SYMBOL_PLAY, PCMD_TOGGLE, 84, ui_accent_hex());
  pl_make_btn(ctl, LV_SYMBOL_NEXT, PCMD_NEXT, 66, 0x2A2A2A);
